...
import com.yourorganization.Shipment.Packaging.PackageCollection;
...
public class StockCollection {
    ...
    private Data.Stock convertToData(Stock stock) {
        Data.Product dataProduct = stock.getProduct().convertToProduct(); // Convert domain Product to Protobuf Product

        return Data.Stock.newBuilder()
                .setProduct(dataProduct)
                .setQuantity(stock.getQuantity())
                .build();
    }

    private Stock convertFromData(Data.Stock dataStock) {
        // Convert the protobuf Data.Product to a domain model Product
        Product product = Product.convertFromData(dataStock.getProduct()); // Static method call
    
        // Create and return a new Stock object using the converted Product and the quantity from Data.Stock
        return new Stock(product, dataStock.getQuantity());
    }
    
    public void save(String filename) {
        List<Stock> stocks = getAllStockItems();
        Data.Stock.Builder stockBuilder = Data.Stock.newBuilder();

        for (Stock stock : stocks) {
            Data.Stock dataStock = convertToData(stock);
            stockBuilder.addStock(dataStock);
        }
        Data.Stock stockCollection = stockBuilder.build();

        try (FileOutputStream output = new FileOutputStream(filename)) {
            stockCollection.writeTo(output);
        }
    }
    ...
    public void load(String filename) {
        try (FileInputStream input = new FileInputStream(filename)) {
            ...
            Data.Stock stockCollection = Data.Stock.parseFrom(input);
            
            this.stocks.clear(); 
    
            for (Data.Stock dataStock : stockCollection.getStocksList()) {
                Stock stock = convertFromData(dataStock); 
