...
public abstract class Transport {
    ...
    private Data.Transport convertToData(Transport transport) {
        // Assuming Data.Transport is the class generated from Protobuf for Transport message
        return Data.Transport.newBuilder()
                .setId(transport.getId())
                .setSpeed(transport.getSpeed())
                .setCostPerKm(transport.getCostPerKm())
                .build();
    }

    private  Transport convertFromData(Data.Transport dataTransport) {
        List<String> routes = Arrays.asList("route1 "," route 2");
        Transport transport= TransportFactory.createTransport("aircraft",dataTransport.getId(),dataTransport.getSpeed(),dataTransport.getCostPerKm(),routes);
        return transport;
    }
